...
fn bench_enforce_law(c: &mut Criterion) {
    let data = BenchData { value: 100, limit: 500 };
    
    // Warmup
    data.enforce_law().unwrap();
    
    c.bench_function("enforce_law_valid", |b| b.iter(|| {
        black_box(&data).enforce_law().unwrap();
    }));
}

fn bench_enforce_law_failure(c: &mut Criterion) {
    let data = BenchData { value: -1, limit: 500 };
    
    c.bench_function("enforce_law_invalid", |b| b.iter(|| {
